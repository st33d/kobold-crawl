...
{
    - inventory ? multipass: Either way it doesn't look like the sort of beast that will recognise the special pendant you carry.
}
-> opt

= opt
{
    - falconsloth_dead:-> navigate
...
+ [Run around the falconsloth.]
    <- set_speed
    {
        ...
        -falconsloth_speed == 2:
            ...
            + + [Try running around again. (%{chance_1})]
                -> run_fast
            + + [Attack the falconsloth. (%{chance_2})]
                -> attack_fast
...
+ [Attack the falconsloth.]
    <- set_speed
    {
        ...
        -falconsloth_speed == 2:
            ...
            + + [Run around the falconsloth. (%{chance_1})]
                -> run_fast
            + + [Continue the attack. (%{chance_2})]
                -> attack_fast
            + + [{backtrack()}]-> go_direction(-1)
    }
+ [{backtrack()}]-> go_direction(-1)
...
= speed1
+ [Walk around the falconsloth. {dirName()}]
    ...
    -> go_direction(1)
+ [Attack the falconsloth. (%95)]
    <- rollChance(95)
    {
        - isSuccess:
            ~ falconsloth_dead = true
            ~ inventory += falconsloth_red
            ...
            You pluck a feather from the corpse and put it in your bag as a trophy.
            It's not clear who in this room is the real monster.
            -> navigate
        - else:
            {You approach the falconsloth with your spear raised, but stop. Are you really doing the right thing?|You still can't do it. Not even a 95% chance will let harm this gentle creature.|Face it. It's not meant to be.}
            -> speed1
    }
...
~ inventory += falconsloth_blue
You pluck a feather from the corpse and put it in your bag, proof that you survived this.
...
You return to the room with the falconsloth.
{
    ...
    -falconsloth_speed == 0:It hangs motionless on its bars, staring you down with its big black eyes.
        {
            - inventory ? multipass: {You show it your pendant. It doesn't even bother to congratulate you.|}
        }
    ...
    -falconsloth_speed == 2:It hangs there not moving. You blink and when you open eyes it is hanging in the same position but much closer.
        {
            - inventory ? multipass: {You hide your pendant, worried it might decide to snatch it from you.|}
        }
}
-> opt
